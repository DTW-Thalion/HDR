...
def test_run_stage_10_output_structure(tmp_path: Path):
    """Stage 10 should produce JSON with correct structure."""
    result = run_stage_10(N_sim=100, T=20, output_dir=tmp_path, fast_mode=False)

    assert "sweep" in result
    assert len(result["sweep"]) == 5  # 5 R_Brier levels

    out_file = tmp_path / "mode_b_fp_fn_sweep.json"
    assert out_file.exists()


def test_run_stage_10_thresholds_equal_at_zero(tmp_path: Path):
...
def test_run_stage_10_fp_rates_in_range(tmp_path: Path):
...
def test_run_stage_10_all_5_levels_present(tmp_path: Path):
