"""
HDR Validation Suite — Extended-512 Profile Runner
===================================================
Exercises all validation stages (01–07) using the extended_512 profile:
  seeds=[101, 202, 303], episodes_per_experiment=10, steps_per_episode=512,
  mc_rollouts=150.
...
Usage:
    python3 extended_512_runner.py
"""
...
# ── Extended-512 profile config (overrides only) ──────────────────────────────
EXTENDED_512_CONFIG: dict[str, Any] = make_config(
    profile_name="extended_512",
    max_dwell_len=512,
    seeds=[101, 202, 303],
...
def stage01_math():
    ...
    # 01.7 T_k_eff formula (longer episodes: T=512)
    T_eff = compute_T_k_eff(T=512.0, pi_k=0.5, p_miss=0.3, rho_k=0.72)
    expected = 512.0 * 0.5 * 0.7 * 0.28
    record("stage01", "T_k_eff formula correct (T=512)", abs(T_eff - expected) < 1e-6,
           f"{T_eff:.4f} (expected {expected:.4f})")

    # 01.8 T_k_eff is exactly double vs T=256 (linear scaling)
    T_eff_256 = compute_T_k_eff(T=256.0, pi_k=0.5, p_miss=0.3, rho_k=0.72)
    record("stage01", "T_k_eff scales linearly with T",
           abs(T_eff - 2 * T_eff_256) < 1e-6, f"{T_eff:.4f} = 2 × {T_eff_256:.4f}")
    ...
    # 01.10 DwellModel PMF — extended max_len=512
    dwell = DwellModel("poisson", {"mean": 10.0}, max_len=512)
    pmf_sum = float(np.sum(dwell.pmf()))
    record("stage01", "DwellModel PMF sums to 1 (max_len=512)",
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 02 — Synthetic episode generation (3 seeds × 10 episodes = 30 total)
# ═══════════════════════════════════════════════════════════════════════════════
def stage02_generation() -> list[dict]:
    ...
    record("stage02", "x_true shape correct (T=512)", all_episodes[0]["x_true"].shape == (T, n),
...
def stage03b_ici(stage03_data: dict) -> None:
    ...
    # 03b.3 T_k_eff with extended episode length (T=512)
    n = cfg["state_dim"]
    n_theta = n * n + n * cfg["control_dim"] + n
    T_eff_256 = compute_T_k_eff(T=512.0, pi_k=0.5, p_miss=0.3, rho_k=0.96)
    T_eff_128 = compute_T_k_eff(T=256.0, pi_k=0.5, p_miss=0.3, rho_k=0.96)
    omega_min = compute_omega_min(n_theta=n_theta, epsilon=0.10, delta=0.05)
    record("stage03b", "T_k_eff (T=512) > T_k_eff (T=256)",
    ...
    # 03b.6 Extended pooled cal sample = 3 seeds × 10 eps × 512 steps
    expected_n_cal = len(cfg["seeds"]) * cfg["episodes_per_experiment"] * cfg["steps_per_episode"]
    record("stage03b", "pooled cal sample = 3×10×512 = 15360",
...
def stage03c_mode_c() -> None:
    ...
    # 03c.5 Fisher proxy from longer trajectories (512 steps) is richer
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 04 — Mode A control (multi-seed pooled, 512-step episodes)
# ═══════════════════════════════════════════════════════════════════════════════
def stage04_mode_a(episodes: list[dict]) -> None:
    ...
    for ep_idx, ep in enumerate(episodes):
        ...
        # HP-1: Pre-generate raw observation noise for hdr_main policy
        ...
        for pol_name in policy_names:
            ...
            for t in range(T):
                if pol_name == "hdr_main":
                    # HP-1: Use pre-drawn noise
                    noise_t = obs_noise_raw[t]
                    mask_t_sched = mask_sched[t]
                    R_t = heteroskedastic_R(basin_R, x, mask_t_sched, t)
                    diag_R_t = np.diag(R_t)
                    y_t = basin_C @ x + basin_c + noise_t * np.sqrt(diag_R_t)
                    y_t = np.where(mask_t_sched.astype(bool), y_t, np.nan)
                    mask_t = (~np.isnan(y_t)).astype(int)
                    y_clean = np.where(np.isnan(y_t), 0.0, y_t)
                ...
                # HP-2: Safety check with pre-computed std
                if _safety_violation_fast(x, basin_obj, basin_idx, seed):
                    violations += 1

                # HP-4: Evolve (E=I elided)
                w = process_noise[t]
                x = basin_A @ x + basin_B @ u + w + basin_b
    ...
    out_dir = ROOT / "results" / "stage_04" / "extended_512"
...
def stage05_mode_b() -> None:
    ...
    start = start_state
    H_test = 12
    mc_rollouts = EXTENDED_512_CONFIG["mc_rollouts"]
    passive_esc, agg_esc = [], []
...
def stage06_coherence() -> None:
    ...
    # Extended: coherence window (24 steps) << episode length (512) — penalty integrable
...
def stage07_robustness() -> None:
    ...
    # 07.2 T_k_eff formula with extended T=512
    for rho_k in [0.72, 0.85, 0.96]:
        T_eff = compute_T_k_eff(T=512.0, pi_k=0.33, p_miss=0.5, rho_k=rho_k)
        expected = 512.0 * 0.33 * 0.5 * (1 - rho_k)
        record("stage07", f"T_k_eff formula rho={rho_k} (T=512)",
...
if __name__ == "__main__":
    print("HDR Validation Suite — Extended-512 Profile")
    print(f"Seeds: {EXTENDED_512_CONFIG['seeds']}")
    print(f"Episodes/seed: {EXTENDED_512_CONFIG['episodes_per_experiment']}")
    print(f"Steps/episode: {EXTENDED_512_CONFIG['steps_per_episode']}")
    print(f"MC rollouts: {EXTENDED_512_CONFIG['mc_rollouts']}")
    ...
    run_stage("Stage 02 — Synthetic Data Generation (3 seeds, T=512)", lambda: None)
    ...
    run_stage("Stage 04 — Mode A Control (3 seeds, T=512)", lambda: stage04_mode_a(episodes))
