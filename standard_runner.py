"""
HDR Validation Suite — Standard Profile Runner
===============================================
Exercises all validation stages (01–07) using the standard profile:
  seeds=[101, 202], episodes_per_experiment=12, mc_rollouts=100.

Data-generating stages (02, 03, 03b, 04) pool results across both seeds.
Seed-independent stages (01, 03c, 05, 06, 07) run once.

Usage:
    python3 standard_runner.py
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 02 — Synthetic episode generation (multi-seed)
# ═══════════════════════════════════════════════════════════════════════════════
def stage02_generation() -> list[dict]:
    ...
    for seed in cfg["seeds"]:
        ...
        print(f"    seed={seed}: generated {n_eps} episodes")
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 03 — IMM inference (multi-seed, pooled)
# ═══════════════════════════════════════════════════════════════════════════════
def stage03_imm(episodes: list[dict]) -> dict:
    ...
    # Run IMM for each seed's episodes (split pooled list back by seed)
    ...
    # 03.4 F1 for mode 1 detection (pooled across seeds)
    ...
    if y_true_bin.sum() > 0:
        ...
        record("stage03", "mode1 F1 > 0 (pooled)", f1 > 0.0, f"{f1:.4f}")
    else:
        record("stage03", "mode1 F1 (no positives in data)", True, "N/A")

    # 03.5 Pooled sample count is double vs smoke
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 03b — ICI diagnostics (uses pooled calibration data)
# ═══════════════════════════════════════════════════════════════════════════════
def stage03b_ici(stage03_data: dict) -> None:
    ...
    # 03b.1 Brier calibration (larger sample → better estimate)
    ...
    # 03b.2 Calibration reduces Brier reliability
    ...
    # 03b.6 Regime boundary: condition_iii fires when T_k_eff too low
    ...
    # 03b.7 Larger calibration sample: Brier estimate is more stable
    # With 2 seeds, pooled n_cal is ~2x smoke => tighter estimate
    record("stage03b", "pooled cal sample size ≥ 2*smoke",
           n_cal >= 2 * 8 * 128, f"n_cal={n_cal}")
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 04 — Mode A control (multi-seed, pooled episodes)
# ═══════════════════════════════════════════════════════════════════════════════
def stage04_mode_a(episodes: list[dict]) -> None:
    ...
    cfg = STANDARD_CONFIG
    # Use first seed's model (evaluation model is deterministic given seed)
    ...
    # Sample from pooled episodes (structural checks)
    ...
    # 04.6 Second seed consistency
    rng2 = np.random.default_rng(cfg["seeds"][1] + 400)
    eval_model2 = make_evaluation_model(cfg, rng2)
    basin2 = eval_model2.basins[0]
    target2 = build_target_set(0, cfg)
    x_test = np.ones(n)
    res2 = solve_mode_a(x_test, np.eye(n)*0.1, basin2, target2, kappa_hat=0.6, config=cfg, step=0)
    record("stage04", "Mode A seed=202 finite u", bool(np.all(np.isfinite(res2.u))))
    ...
    # Pooled LQR: single gain from basin-averaged dynamics
    ...
    # Basin-specific LQR gains (oracle)
    ...
    for ep_idx, ep in enumerate(episodes):
        ...
        # Start from a non-trivial state: use mid-episode state from the
        # original open-loop trajectory so the controller has work to do.
        t_start = min(T // 4, T - 1)
        x_init = ep["x_true"][t_start].copy()

        # Pre-generate process noise and observation schedule shared across policies
    ...
    # Bootstrap 95% CI for maladaptive gains (Benchmark A)
    ...
    # --- Basin-stratified diagnostics ---
    basins_arr = np.array(ep_basins)
    print(f"\n    --- Basin-stratified diagnostics ---")
    print(f"    Basin distribution: " + ", ".join(
        f"basin {b}: {int(np.sum(basins_arr == b))}" for b in range(3)))

    print(f"\n    Paired gain (pe-hdr)/pe by basin:")
    print(f"    Basin | Count | Mean gain  | Median gain")
    print(f"    ------|-------|------------|------------")
    for b in range(3):
        mask_b = basins_arr == b
        if not np.any(mask_b):
            print(f"      {b}   |   0   |     —      |     —")
            continue
        ratios_b = paired_ratios[mask_b]
        print(f"      {b}   | {int(np.sum(mask_b)):>5} | {np.mean(ratios_b):>+10.6f} | {np.median(ratios_b):>+10.6f}")

    print(f"\n    HDR-winning episodes (pe cost > hdr cost):")
    win_mask = cost_diff > 0
    if np.any(win_mask):
        win_basins = basins_arr[win_mask]
        for b in range(3):
            print(f"      basin {b}: {int(np.sum(win_basins == b))} episodes")
    else:
        print(f"      (none)")

    print(f"    HDR-losing episodes (pe cost < hdr cost):")
    lose_mask = cost_diff < 0
    if np.any(lose_mask):
        lose_basins = basins_arr[lose_mask]
        for b in range(3):
            print(f"      basin {b}: {int(np.sum(lose_basins == b))} episodes")
    else:
        print(f"      (none)")
    ...
    # --- Mode-error regression: cost degradation vs mu_hat sweep ---
    ...
    # --- Target-drift regression: cost vs drift magnitude ---
    ...
    cal_results = {
        ...
        "benchmark_a_ci_lo": ci_lo,
        "benchmark_a_ci_hi": ci_hi,
...
def stage05_mode_b() -> None:
    ...
    # 05.6 MC escape prob with standard rollouts (100)
    rng = np.random.default_rng(42)
    P_passive = P_actions["conservative"]
    P_aggressive = P_actions["aggressive"]
    start = start_state
    H_test = 12
    mc_rollouts = STANDARD_CONFIG["mc_rollouts"]
    passive_esc, agg_esc = [], []
...
def stage06_coherence() -> None:
    ...
    #
    # Two sub-tests that reflect HDR's clinical design intent:
    #
    # (a) Normal basin (basin 0, rho=0.72): non-inferiority vs open-loop
    #     HDR should conserve its burden budget in healthy regimes — not
    #     intervene aggressively when the patient is already well-regulated.
    #     Criterion: delta >= -0.05  (within 5 pp of open-loop)
    #
    # (b) Maladaptive basin (basin 1, rho=0.96): superiority vs open-loop
    #     AND non-inferiority vs burden-constrained LQR.
    #     HDR must actively improve time-in-band where remediation is needed.
    #     The LQR baseline is given the identical per-episode burden budget so
    #     the comparison is intervention-cost-matched, not unconstrained.
    from hdr_validation.model.slds import make_evaluation_model
    from hdr_validation.control.mpc import solve_mode_a
    from hdr_validation.control.lqr import dlqr
    from hdr_validation.model.target_set import build_target_set
    ...
    def _tib_rollouts(basin, n_eps, seed_base):
        ...
        for ep_i in range(n_eps):
            ...
            for t in range(T_tib):
                ...
                u_hdr = res_hdr.u
                used_burden_hdr += float(np.sum(np.abs(u_hdr)))
                # Budget-constrained LQR: proportionally scale when remaining
                # budget is insufficient to apply the full unconstrained gain.
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 07 — Robustness sweeps (seed-independent)
# ═══════════════════════════════════════════════════════════════════════════════
def stage07_robustness() -> None:
    ...
    # 07.2 T_k_eff regime boundary: compound bound formula
    ...
    # 07.6 w3 sweep: peak time-in-band across w3 values
    ...
    T_w3 = min(cfg["steps_per_episode"], 32)
...
if __name__ == "__main__":
    print("HDR Validation Suite — Standard Profile")
    print(f"Seeds: {STANDARD_CONFIG['seeds']}")
    print(f"Episodes/seed: {STANDARD_CONFIG['episodes_per_experiment']}")
    print(f"MC rollouts: {STANDARD_CONFIG['mc_rollouts']}")
    ...
    # Multi-seed data generation
    episodes = stage02_generation()
    run_stage("Stage 02 — Synthetic Data Generation (2 seeds)", lambda: None)

    # IMM over pooled episodes
    stage03_data = stage03_imm(episodes)
    run_stage("Stage 03 — IMM Inference (pooled)", lambda: None)
    ...
    run_stage("Stage 04 — Mode A Control (multi-seed)", lambda: stage04_mode_a(episodes))
