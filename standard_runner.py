...
def stage04_mode_a(episodes: list[dict]) -> None:
    ...
    for ep_idx, ep in enumerate(episodes):
        ...
        for pol_name in policy_names:
            ...
            for t in range(T):
                # --- Generate observation from current state (for HDR) ---
                if pol_name == "hdr_main":
                ...
                # --- Accumulate cost: ||x||^2 + lambda_u * ||u||^2 ---
                cost_accum += float(np.dot(x, x) + lambda_u * np.dot(u, u))

                # --- Safety check (consistent covariance for all policies) ---
                if _safety_violation(x, basin_obj):
                    violations += 1

                # --- Evolve state with shared process noise ---
                w = process_noise[t]
